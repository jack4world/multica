...
审计垂直（`docs/audit/CONTEXT.md`、ADR-0002）对三个平台名词采用不同的渲染。这张表是 contract 的一部分：审计模式下遵循它的文案是正确的，回退到基础词汇表的文案是 bug。

覆盖**只发生在 i18n 层**。代码标识符、数据库列、API 字段和路由在任何地方都仍然是 `issue`、`workspace`、`project`。

| 平台名词 | 基础词汇表 | 审计模式（en） | 审计模式（zh-Hans） | 生效范围 |
| --- | --- | --- | --- | --- |
| `workspace` | Workspace / 工作区 | **Auditee** | **被审计单位** | 审计模式下全局 |
| `project` | Project / 项目 | **Engagement** | **审计项目** | 审计模式下全局 |
| `issue` | Issue / 任务 | **Workpaper** | **工作底稿** | 审计项目范围内的界面 |
| `issue` | Issue / 任务 | **Remediation Item** | **整改事项** | 整改看板与列表 |

`issue` 无法全局覆盖：处在审计项目内的 issue 是工作底稿，不属于任何审计项目的 issue 是整改事项。覆盖必须按界面区域划分。

### 不覆盖的词

以下在审计模式下保持基础词汇表的渲染。显式列出，避免反复讨论：

`agent`（智能体）、Run（运行）、`skill`（保持小写英文）、`comment`（评论）、`label`（标签）、`member`（成员）、`inbox`（收件箱）、`autopilot`（自动化）、**`assignee`（负责人）**。

`assignee` 值得单独说明。一份工作底稿在编制阶段的负责人是编制人，进入复核后是复核人；"负责人"作为通名同时成立，覆盖成其中任何一个，都会在另一个阶段说谎。

### 语言范围

覆盖表提供 `en` 与 `zh-Hans`。`ja` / `ko` 回落到基础词汇表——审计垂直在这两个市场没有业务，半翻译的审计词汇比不翻译更糟。

### seed 出来的目录名不是 i18n 字符串

复核链状态与审计属性定义是每个工作区 seed 一次的数据库行（`server/internal/auditmode`）。它们在启用审计模式时按调用方指定的语言写入，此后可被用户编辑；改这张表不会重写它们。
