...
**审计档案 (Engagement Archive)**:
One finished engagement written out as a complete, indexed set of files: the issued report, the workpapers behind it, the evidence index, the trail, and the items it raised — with a manifest carrying a digest of each. Written once, at close. Distinct from 归档, which is a workpaper reaching its terminal state.
_Avoid_: 归档 (which is about a workpaper), 备份, backup
